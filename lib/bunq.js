...
async function bunqFetch(method, path, { body, authToken, privateKeyPem } = {}) {
  ...
  if (!res.ok) {
    const msg = json?.Error?.[0]?.error_description || text || res.statusText;
    throw new Error(`bunq ${method} ${path} faalde (${res.status}): ${msg}`);
  }
...
export async function ensureContext({ apiKey, ctx, save }) {
  ...
  if (verlopen) {
    ...
    const user = pick(resp, 'UserCompany') || pick(resp, 'UserPerson') || pick(resp, 'UserApiKey');
