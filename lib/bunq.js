...
export function bunqBase() { return BASE; }
...
export async function ensureContext({ apiKey, ctx, save }) {
  ...
  if (verlopen) {
    ...
    // Echte gebruiker bepalen. Bij een API-key-sessie zit de gebruiker vaak
    // genest onder UserApiKey.requested_by_user.
    let user = pick(resp, 'UserPerson') || pick(resp, 'UserCompany');
    let type = user ? (pick(resp, 'UserPerson') ? 'UserPerson' : 'UserCompany') : null;
    if (!user) {
      const apiKeyUser = pick(resp, 'UserApiKey') || pickUser(resp);
      if (apiKeyUser) {
        const rb = apiKeyUser.requested_by_user || {};
        user = rb.UserPerson || rb.UserCompany || apiKeyUser;
        type = rb.UserPerson ? 'requested UserPerson' : rb.UserCompany ? 'requested UserCompany' : 'UserApiKey';
      }
    }
    context.session_user_id = user?.id || null;
    context.session_user_type = type;
...
export async function getUserId(context) {
  const resp = await bunqFetch('GET', '/v1/user', {
  ...
  for (const item of resp) {
    const key = Object.keys(item)[0];
    const u = item[key];
    if (u && u.id && key !== 'UserApiKey') return u.id;
  }
  for (const item of resp) {
    const u = item[Object.keys(item)[0]];
    if (u?.id) return u.id;
  }
  return context.session_user_id;
}

export async function listMonetaryAccounts(context, userId) {
  const uid = userId || context.session_user_id;
  const accounts = [];
  let olderId = null;
  for (let p = 0; p < 10; p++) {
    const q = `count=200${olderId ? `&older_id=${olderId}` : ''}`;
    const resp = await bunqFetch('GET', `/v1/user/${uid}/monetary-account?${q}`, {
      authToken: context.session_token,
      privateKeyPem: context.private_key_pem,
    });
    ...
    if (resp.length < 200) break;
    olderId = laatsteId;
  }
  return accounts;
}

export async function listPayments(context, userId, accountId, { sinds = null, maxPaginas = 25 } = {}) {
  const uid = userId || context.session_user_id;
  const uit = [];
  let olderId = null;
  for (let p = 0; p < maxPaginas; p++) {
    ...
    const resp = await bunqFetch(
      'GET',
      `/v1/user/${uid}/monetary-account/${accountId}/payment?${q}`,
