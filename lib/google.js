...
// Verzamelt PDF-bijlagen uit Gmail-mails onder een label. Geeft {bron_id, bestandsnaam, tegenpartij, datum, bytes}.
export async function gmailFacturen({ label, maxMails = 25, sindsDagen = 120 }) {
  const token = await accessToken();
  const q = encodeURIComponent(`label:${label} has:attachment filename:pdf newer_than:${sindsDagen}d`);
  const lijst = await gapi(token, `https://gmail.googleapis.com/gmail/v1/users/me/messages?q=${q}&maxResults=${maxMails}`);
  const resultaten = [];
