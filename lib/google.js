...
async function accessToken() {
  ...
  if (!res.ok) throw new Error(`Google token faalde (${json.error || res.status}): ${json.error_description || ''}`);
...
async function gapi(token, url) {
  for (let poging = 0; poging < 4; poging++) {
    ...
    if ((res.status === 429 || res.status === 403 || res.status === 503) && poging < 3) {
      const txt = await res.text();
      if (/rate|quota|userRateLimit|limitExceeded/i.test(txt) || res.status === 429 || res.status === 503) {
        await new Promise((r) => setTimeout(r, 1000 * (poging + 1)));
        continue;
      }
  ...
  throw new Error('Google API: te vaak rate limited');
}
...
// Haalt de PDF-bijlagen van een enkel bericht op.
async function bijlagenVanBericht(token, id) {
  const bericht = await gapi(token, `https://gmail.googleapis.com/gmail/v1/users/me/messages/${id}?format=full`);
  const heads = bericht.payload?.headers || [];
  const tegenpartij = afzenderNaam(header(heads, 'From'));
  const datum = new Date(Number(bericht.internalDate || Date.now())).toISOString().slice(0, 10);
  const parts = [];
  ...
  const out = [];
  for (const p of parts) {
    const att = await gapi(token, `https://gmail.googleapis.com/gmail/v1/users/me/messages/${id}/attachments/${p.body.attachmentId}`);
    out.push({
      bron: 'gmail', bron_id: `gmail:${id}:${p.body.attachmentId}`,
      bestandsnaam: p.filename, tegenpartij, datum, bytes: Buffer.from(att.data, 'base64url'),
    });
  }
  return out;
}

// Eén pagina van een brede sweep. Geeft de items plus de volgende cursor terug.
export async function gmailSweepPagina({ query, pageToken, maxMails = 30 }) {
  const token = await accessToken();
  const u = `https://gmail.googleapis.com/gmail/v1/users/me/messages?q=${encodeURIComponent(query)}&maxResults=${maxMails}${pageToken ? `&pageToken=${pageToken}` : ''}`;
  const lijst = await gapi(token, u);
  const items = [];
  for (const m of lijst.messages || []) {
    const b = await bijlagenVanBericht(token, m.id);
    for (const x of b) items.push(x);
  }
  return { items, nextPageToken: lijst.nextPageToken || null };
}
